...
"""
learn_pipeline.py
统一 learn 流程入口：sync → diff → promote → generate candidate → eval。
...
def learn_pipeline(child_name: str, factory_dir: str, project_root: str, article_id: str | None, skip_eval: bool, dry_run: bool) -> dict:
    ...
    # ------------------------------------------------------------------
    if dry_run:
        print(f"[DRY-RUN] 将执行 sync_visible_edits({child_name})")
        results["steps"].append({"step": "sync", "status": "dry_run"})
    else:
        print(f"[learn_pipeline] Step 1/5: Syncing visible edits...")
        ...
        print(f"[learn_pipeline] Sync result: {sync_result.get('status')}, changes: {len(sync_result.get('changes', []))}")

        # 自动 git commit sync 结果
    ...
    if article_id:
        ...
        # 从 sync 结果中提取 edit 类型的 article_id
    ...
    if not target_article_ids and not article_id:
        print("[learn_pipeline] No edited articles found. Skipping diff & promote.")
    ...
    if dry_run:
        for aid in target_article_ids or ["<detected>"]:
            print(f"[DRY-RUN] 将执行 promote_rules({child_name}, {aid})")
        results["steps"].append({"step": "promote", "status": "dry_run"})
    else:
        print(f"[learn_pipeline] Step 3/5: Promoting rules for {len(target_article_ids)} article(s)...")
        for aid in target_article_ids:
            promote_result = run_promote(child_name, aid, factory_dir)
            promote_results.append(promote_result)
            print(f"[learn_pipeline] Promote {aid}: {promote_result.get('status')}, promoted={len(promote_result.get('promoted', []))}")

        # 自动 git commit promote 结果
        if promote_results:
    ...
    # ------------------------------------------------------------------
    if dry_run:
        print(f"[DRY-RUN] 将执行 generate_candidate({child_name})")
        results["steps"].append({"step": "generate_candidate", "status": "dry_run"})
    else:
        print(f"[learn_pipeline] Step 4/5: Generating candidate...")
        ...
        print(f"[learn_pipeline] Candidate generated: {candidate_result.get('version', 'unknown')}")

    # ------------------------------------------------------------------
    # Step 5: Eval
    # ------------------------------------------------------------------
    if skip_eval:
        print(f"[learn_pipeline] Step 5/5: Skipping eval (--skip-eval)")
        results["steps"].append({"step": "eval", "status": "skipped"})
    elif dry_run:
        print(f"[DRY-RUN] 将执行 eval_candidate({child_name})")
        results["steps"].append({"step": "eval", "status": "dry_run"})
    else:
        print(f"[learn_pipeline] Step 5/5: Evaluating candidate...")
        eval_result = run_eval(child_name, factory_dir)
        results["steps"].append({"step": "eval", "status": "done", "eval": eval_result})
        print(f"[learn_pipeline] Eval total score: {eval_result.get('total_score', 'N/A')}")
    ...
    print("[learn_pipeline] Pipeline complete.")
    return results
