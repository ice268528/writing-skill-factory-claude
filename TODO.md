...
### 1. 父 skill 骨架搭建
- [ ] 创建 `.claude/skills/writing-skill-factory-cli/` 目录结构
  - `SKILL.md`
  - `references/`（architecture.md, source-mapping.md, style-dimensions.md, update-policy.md, eval-rubric.md, version-policy.md）
  - `templates/`（child-skill-skill.md.tpl, style-summary.md.tpl, usage-guide.md.tpl, manifest.json.tpl, evals.json.tpl）
  - `scripts/`（全部 .py 占位文件）
- [ ] 编写父 skill `SKILL.md`（触发条件、6 大指令映射、参数说明）
- [ ] 编写 `references/architecture.md`（系统架构与数据流）

### 2. create-child 核心链路
- [ ] 实现 `scripts/create_child.py`
...
- [ ] 实现 `scripts/build_cognitive_profile.py`
  - 心智模型、决策启发式、反模式、诚实边界
...
### 3. 子 skill 最小文件集
- [ ] 生成 `skill/SKILL.md`（frontmatter + 触发条件 + 写作流程 + supporting files 导航）
- [ ] 生成 `references/style-profile.md`
- [ ] 生成 `references/editorial-rules.md`
- [ ] 生成 `references/author-boundary.md`
- [ ] 生成 `references/anti-patterns.md`
- [ ] 生成 `references/examples.md`（3-5 段 few-shot）
- [ ] 生成 `assets/style-memory.json`
- [ ] 生成 `assets/quality-rubric.json`
- [ ] 生成 `evals/evals.json`

### 4. active child 发布
- [ ] 实现 `scripts/publish_child.py`
...
- [ ] 生成 sidecar manifest（`.manifest/<article_id>.json`）
  - 字段：article_id, child_name, child_version, baseline_sha, created_at, topic, status
- [ ] 实现文章对齐机制（article_id + baseline_sha + manifest）
...
### 10. 用户决策与发布
- [ ] 在父 skill 中集成 eval 结果展示
- [ ] 用户确认 publish 后，调用 `publish_child.py` 发布 candidate
- [ ] 用户拒绝时，保留 candidate 与报告，不发布
...
### 11. release 管理
- [ ] 完善 `state/release-index.json` 结构
- [ ] 规范 release tag 命名（semver）
- [ ] 生成 release notes（`reports/change-log.md`）
- [ ] create 阶段生成 `reports/create-summary.md`

### 12. rollback
- [ ] 实现 `scripts/rollback_child.py`
  - 按版本 tag 回退 canonical repo
  - 重新 publish 到 active child
  - 保留被回滚的 candidate 报告

### 13. 评估体系深化
- [ ] 完善 `references/eval-rubric.md`
- [ ] 细化 `assets/quality-rubric.json` 评分标准
- [ ] 增加更多自动化回归测试场景
- [ ] 完善 `evals/evals.json`（素材成稿类、风格迁移类、边界判断类）
...
- [ ] 完善 `references/update-policy.md`
- [ ] 完善 `references/version-policy.md`
- [ ] 增加错误处理与边界 case 日志
...
- [ ] child skill 输出是多文件 skill，不是单段 prompt
- [ ] child skill 至少具备：选题判断、文章原型识别、AI/人类边界控制、作者态二改、最终自检
